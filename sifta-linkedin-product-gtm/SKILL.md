...
3. 用户明确要求执行且只要 1-3 个强线索时，运行 `node scripts/small-batch-product-gtm.mjs --query "<用户语言画像>" --checkpoint "<用户原始目标>" --target-count 3`。辅助脚本完成后直接整理用户报告，不再追加网页/Exa/公司核验、浏览器查询或第二次 `find-people`。如果辅助脚本失败、未认证、API 不可达或返回 0 人，不要把失败冒充成 connector 成功，也不要用公开网页、Exa、浏览器、公司页、活动页、GitHub/X 社区证据替换候选人；本轮只能交付来源地图、产业标杆、待核验线索、阻塞项和下一步修复动作，不输出全职候选、strong/soft shortlist 或“建议先聊”的候选人分桶。不要输出停止标记、脚本名、命令、参数、运行过程、`target-count` 或使用了哪个 skill，也不要编造候选人分桶。
...
9. 输出候选人分桶和适配证明包；产品/GTM 候选人也必须有职业证据和风险。
...
- 辅助脚本失败、未认证、API 不可达或 0 人时，明确 connector 主路径失败；本轮只输出来源地图、产业标杆、待核验线索、阻塞项和下一步修复动作，不用公开网页/社区/活动页 fallback 形成候选名单。
...
- 最终输出链接列必须保留 `[资料](url)`、`[领英](url)` 或 `[主页](url)` Markdown 链接；不要把 helper 返回的链接改成裸 URL。
