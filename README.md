...
| 目标                    | 可以这样说                                                              |
| ----------------------- | ----------------------------------------------------------------------- |
| 找 GitHub 工程人选      | `帮我找 10 个做过 MCP / Agent 基础设施的开源工程师，优先有真实仓库证据` |
| 找产品 / GTM / 增长人选 | `找几个做过 AI 开发者工具增长或商业化的人，偏早期团队经验`              |
| 找研究型人才            | `帮我找 WAM/VLA 方向有论文和工程实现的年轻研究人才`                     |
| 找大模型工程师          | `找几个 Qwen/DeepSeek/vLLM 方向的大模型训练或推理核心工程师，优先有开源证据` |
| 盘点创始人 / 超级个体   | `盘点几个 AI Coding/Agent 方向有出圈作品的技术型创始人或独立开发者`     |
...
| 模块         | 内容                                                 |
| ------------ | ---------------------------------------------------- |
| 本轮目标     | 原始目标、假设、必要条件、排除项                     |
| 找人来源     | 用过的来源、待补来源、线索为什么相关、下一步怎么核验 |
| 推荐名单     | 全职候选、顾问/推荐人、产业标杆、待确认或排除项      |
| 匹配依据     | 要求 -> 证据 -> 来源 -> 置信度 -> 风险 -> 下一步     |
| 本轮覆盖缺口 | 召回不足、证据弱、来源失败、分类不确定等风险         |
| 下一步       | 下一轮扩展、人工反馈、为什么先停在这里或触达草稿建议 |

如果某个人只有 repo、论文、公司新闻或项目页线索，但没有可核验的个人资料，应该留在
“找人来源”或“待确认线索”，不能直接当作候选人推荐。

## 4. 能力边界

| 场景                       | 推荐方式                                                                                      |
| -------------------------- | --------------------------------------------------------------------------------------------- |
| GitHub 工程人选            | 先用宿主 Agent 的 GitHub 搜索 / GitHub MCP / `gh`；需要 Sifta 调用轨迹或反馈闭环时用 CLI      |
| 学术图谱 / 学术通道        | 先用宿主 Agent 的学术 / 网页搜索建找人来源；CLI 只用于研究调用轨迹、结构化结果或反馈闭环      |
| LinkedIn 产品、GTM、商业化 | 优先使用 Sifta CLI/API 连接器                                                                 |
| 已知候选人深挖             | 由宿主 Agent 读取公开 profile、主页、GitHub、LinkedIn、X、论文和媒体材料后按 Sifta 质量门整理 |
