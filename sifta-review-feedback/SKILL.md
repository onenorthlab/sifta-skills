...
description: >
    ...
    把某人移入顾问/推荐人/产业标杆，排除候选人类型，或希望下一轮保留约束和还要确认事项
    时使用。
    普通总结、候选人表美化或无上一轮结果的新搜索不要使用；新搜索应回到 `sifta-search` 路由。
---
...
最终答复给招聘负责人看，不是反馈 JSON、命令计划或九段审查表。默认按 `../sifta-search/templates/final-report.md` 输出短决策简报：

```text
结论 → 人选和证据 → 其他线索 / 需要确认 → 下一步
```

内部防漏检查不是用户可见版式。项目简报、来源地图、候选人分层、待核验线索、适配证明、覆盖缺口、停止原因、执行边界、下一步必须折进短报告里，不得渲染成九个顶层标题。

输出要求：

- `结论`：说明接受了哪些人工反馈、排除了哪些方向、本轮是否产生可推进人选。
- `人选和证据`：用招聘视角表格表达 `人选 / 招聘判断 / 为什么值得聊 / 相关作品或证据 / 还要确认 / 下一步`；人选名字必须可点击。
- `其他线索 / 需要确认`：合并呈现已排除或降级的人、反馈约束和下一步补证事项；不要默认展示内部标签。
