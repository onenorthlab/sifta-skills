...
`默认地域/市场：中国/中文生态相关人才池优先`。不要把默认地域/市场理解成族裔、国籍或身份属性；执行时只看公开职业资料中的中国大陆、港澳台、中文教育/工作/社区、中国市场或中国相关机构/公司信号，不凭姓名、照片、外貌、口音、族裔、国籍或猜测判断。默认地域对来源地图是来源优先级，对候选人分桶是升级门槛：缺少公开中国/中文生态相关信号的人可以保留为来源地图线索、产业标杆或待核验对象，但不能包装成候选人或强线索；用户明确放宽为全球人才池时除外。
...
执行类找人请求先在内部做轻量 `Persona Compiler`，再搜索，不把用户原话直接当 query。这个编译过程不暴露给用户，但必须影响来源选择、候选人分桶和下一步核验：
...
| `channel_fit`            | 每类来源适合证明什么、不适合证明什么   | 解释覆盖风险和下一轮补证                            |
...
Product/GTM/LinkedIn 主路径失败时，不输出全职候选、strong/soft shortlist 或“建议先聊”的候选人分桶；只能输出产业标杆、来源地图线索、待核验线索、阻塞项和修复动作。不要用网页、Exa、浏览器、公司页、活动页、GitHub/X 社区证据替换本轮候选人。其他通道若用户明确授权 fallback，也必须写 `coverage=partial`，并说明 fallback 名单不能证明 provider healthy，不能把 fallback 摘要升成 hard strong。
...
4. 如果本轮是计划请求，只规划来源地图，不调用网页搜索、浏览器、CLI 或实时验证；说明下一步候选人核验路径、证据获取路径与覆盖风险后停止。
5. 选择执行面：宿主原生搜索足够时按 Sifta 质量门交付；需要连接器、调用轨迹或反馈闭环时调用 CLI；小批量执行必须遵守执行预算。辅助脚本完成后，内部停止执行并直接整理用户报告；最终答复保留覆盖风险和下一步，不输出停止标记、命令、events、timing、target-count、脚本名或使用了哪个 skill，也不得用网页/浏览器/原生搜索替换候选人。
...
9. 最终输出必须包含适配证明包：要求 -> 证据 -> 来源 -> 置信度 -> 风险 -> 下一步。
...
- **人选和证据**：每位写「分桶 / 人选或线索 / 招聘判断 / 为什么值得聊 / 把握(高中低) / 证据来源 / 下一步 / 链接」。论文、仓库、公司团队这类不能直接当人选；默认中国人才池下，缺公开中国/中文生态职业信号的人不得进入可推进候选。
- **风险和待核验**：合并写来源地图缺口、待核验线索、覆盖风险、本轮为什么停、没有做哪些越界动作。不要拆成多个顶层标题让招聘负责人逐段阅读。
- **下一步**：给出 1-3 个具体招聘动作，例如扩大同来源召回、核某人职业资料、写低压触达草稿。

内部仍必须覆盖九项检查：项目简报、来源地图、候选人分桶、待核验线索、适配证明包、覆盖风险、停止条件、执行合同和下一步；这些内容折进上面 4 段，不作为用户可见标题。

- 执行类报告的「结论」或「风险和待核验」必须显式写出默认地域/市场边界：`中国/中文生态相关人才池优先；只看公开职业信号，不凭姓名、照片、族裔或国籍推断`。
- 执行类报告的「风险和待核验」必须显式写出隐私和触达边界：不查询私人邮箱、手机号、非公开联系方式，不自动发送消息或提交表单。
