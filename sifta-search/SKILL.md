...
| 增长、商业化、出海、开发者生态、开发者社区 | Product/GTM 来源地图；执行时优先 LinkedIn/职业资料，并用公司/社区信号补证据 | 用户不知道头衔不阻塞；头衔地图只是内部扩展，不当主输出锚点          |
...
| 创始人型 / 超级个体 / 独立开发者          | 读 AI 垂直来源地图和角色证明标准，再按主能力走 GitHub、Product/GTM 或学术路径 | 区分可直接联系、顾问/推荐人、产业标杆和待核验线索                    |
...
9. 最终输出必须包含适配证明：要求 -> 证据 -> 来源 -> 置信度 -> 风险 -> 下一步。
...
- **候选人分桶**：每位写「分桶 / 现在做什么 / 为什么值得聊 / 把握(高中低) / 还要确认 / 链接」。论文、仓库、公司团队这类不能直接当候选人。
- **待核验线索**：暂不算正式人选的论文 / 项目 / 公司团队线索，写为什么相关、还差什么、下一步怎么确认。
- **适配证明包**：把用户要求、公开证据、把握、风险和下一步核验动作对应起来。
