...
- 学术图谱：宿主 Agent 可优先使用 OpenAlex、Google Scholar、Semantic Scholar、
  arXiv、OpenReview、Papers with Code、实验室主页、项目页和 GitHub 构造来源地图；
  Google Scholar 只作为浏览器/人工广泛召回或经批准第三方入口，不假设官方 API；
  Sifta CLI 主要用于研究调用轨迹、候选人结构化和反馈闭环。
...
`update` 里只出现有更新的组件版本。Agent 解析候选人结果时应忽略 `_notice.update`，优先读取
...
`sources=github`；这些容易让本地 Agent 复制成非结构化参数。
...
- 研究来源地图：先保留 paper/lab/company/project/repo/dataset 来源地图线索，再转候选人渠道。
- 学术图谱寻访：来源地图可由宿主 Agent 原生学术 / 网页搜索建立；使用 CLI 时
  `--mode research` 只作为结构化调用轨迹和连接器输入，不是唯一执行路径。应说明是否综合
