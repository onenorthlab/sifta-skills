...
用户强调代码、开源、SDK、框架、runtime、observability、agent infra 时，优先看 GitHub。
...
```bash
sifta-cli find-people --query "视频大模型工程师，多模态生成，公开项目或论文证据" \
...
```bash
sifta-cli find-people --query "上海 AI Agent 工程师，偏 infra，有开源项目" --target-count 10
```
...
```bash
sifta-cli find-people --query "中国 AI 工程师，有 agent、LLM 平台或公开项目经验" --target-count 10
```
