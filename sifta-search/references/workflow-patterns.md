...
这些模式用于指导 Agent 在 AI 行业垂直招聘寻访场景中选择命令、组织查询和解释结果。
Sifta 当前只覆盖候选人搜索、候选人档案和候选人评估；不要扩展成公司情报、销售线索、
ATS 管理、触达或 KOL 合作。

宿主 Agent 已经具备通用搜索、网页阅读、论文/公司资料检索等能力时，先用宿主能力做
找人来源，再用 Sifta 搜索候选人。不要把 Sifta 当成通用网页搜索；Sifta 的价值是
候选人渠道、公开个人资料证据、项目内判断和结构化输出。
...
- [6. 研究型人才 / 项目感知寻访](#6-研究型人才--项目感知寻访)
- [7. 已知个人资料或 handle 核验](#7-已知个人资料或-handle-核验)
- [8. 搜索后筛选](#8-搜索后筛选)
...
- 优先引用 GitHub 仓库、贡献、项目说明、个人资料摘要和 `matchReasons`。
...
通常同等重要，但不要把同一个查询同时打到 GitHub 和 LinkedIn。先按来源拆成
GitHub 工程证据或 LinkedIn 职业资料；论文证据明显重要时使用研究模式。
...
sifta-cli find-people --query "embodied AI robotics VLA researcher engineer paper evidence" \
  --checkpoint "找具身智能方向研究型或工程型人才，优先论文和公开个人资料证据" \
  --mode research \
  --target-count 10
```
...
sifta-cli find-people --query "AI startup founder co-founder building AI agent product public profile" \
  --checkpoint "找 AI startup founder/co-founder，自己在做 AI agent 产品，有公开个人资料" \
  --sources '["linkedin"]' \
  --target-count 10
```
...
- Qwen、字节等团队线索需要公开个人资料支撑；没有明确证据时写成“可能相关”。
...
如果用户给出公开 handle，可用补全功能补公开内容信号。

如果目标是 AI video、AI animation、AI avatar、creator tool、全球增长、商业化或 partnerships，
不要直接搜一句泛化候选人查询。先用宿主 Agent 搜相似公司/竞品/相邻赛道，形成公司地图，
...
如果用户明确要求 X / 公开帖子 / 近期社区表达，可以单独跑 X connector 补近期公开信号：

```bash
sifta-cli find-people \
  --query "中国/中文生态 AI developer marketing、开发者社区增长或 DevRel 负责人，X 上近半年有公开产品发布、开发者社区、中国市场或 Agent 生态讨论证据" \
  --checkpoint "明确授权使用 X 公开帖子找 AI developer marketing、社区增长或 DevRel 人才；只看公开职业信号，不抓私信或私人联系方式。" \
  --input '{"sources":["x"],"sourceOptions":{"x":{"fromDate":"2026-01-01","toDate":"2026-06-30"}},"targetCount":3}'
```

X-only 人选缺中国/中文生态公开信号、LinkedIn、个人主页、作品或明确职业资料时，只能作为
待核验线索；不要升级成 `建议优先聊`。
...
- `coauthor-graph`：共同作者、学生、导师、同实验室成员、共同项目页成员。
...
- Google Scholar：广泛召回、漏召回补充、作者主页和引用入口；不要写成官方 API 连接器。
- Semantic Scholar：paper / author search、citation/reference graph、related papers。
- arXiv / OpenReview：预印本、会议投稿、评审场景和前沿论文。
- Papers with Code / 项目页：task、benchmark、dataset、official code 和 repo link。
- 实验室 / 个人主页：个人身份、当前阶段和候选人转换。
...
sifta-cli find-people --query "speech model researcher LLM paper author engineer public profile" \
  --checkpoint "找语音模型或 LLM 方向研究型人才，有论文作者和公开个人资料证据" \
  --mode research \
  --target-count 10
```
...
```bash
sifta-cli find-people \
  --query "WAM VLA 具身智能 机器人数据 仿真 技术合伙人 Head of WAM 中文生态 中国市场，找人来源包含论文、项目、公司和实验室线索" \
  --checkpoint "某具身智能公司要找 WAM 技术合伙人或 Head of WAM，中国/中文生态相关人才池优先，必须有 WAM/VLA/robot data/simulation 至少一项强证据。请先拆找人来源，再从论文、项目、公司和开源证据里找候选人，并区分全职候选、顾问推荐人和产业标杆。" \
  --mode research \
  --target-count 10
```

这类 `--mode research` 的查询是找人来源种子，不等于最终候选人渠道查询。转入
GitHub 时改写成英文技术/角色词；转入 LinkedIn 时保留中文职业画像。

验证方式：

- 区分论文作者证据、工程个人资料证据和招聘可用性。
- 如果只找到论文作者但没有公开职业/个人资料线索，说明它只是研究线索。
- 不要编造邮箱、机构归属或当前在职状态。
- 强学术 PI、头部 founder 或产业标杆不一定是全职候选；应放入顾问推荐人池、产业标杆池或 warnings，并解释 `whyNot`。
- 学术图谱高潜通道下，年轻、博士、顶会论文或竞赛奖项只是入口信号；必须核验个人
  资料、贡献强度、公开实现或职业阶段。
- 学术图谱输出应说明是否综合 OpenAlex、Google Scholar、Semantic Scholar 或同级
  广泛图谱 / 广泛召回来源；没有覆盖时写覆盖风险。
...
转候选人档案路径，由宿主 Agent 读取公开 profile 和可追溯材料。

只知道姓名时，尽量补充公司、地点或画像以减少重名；没有足够消歧线索时只给低置信度说明或向用户要一个 profile，不跨渠道硬合并。

验证方式：

- 只汇报公开来源中存在的个人资料、证据和公开职业联系方式。
- 不要把重名候选人当作同一人。
- 如果个人资料不是 AI 行业招聘画像，直接说明不匹配。

## 8. 搜索后筛选

先用 `find-people` 得到候选人，再基于返回证据做人工式筛选。
...
3. 放宽地点，例如从城市放宽到国家、地区或远程。
4. 同源调整后仍弱时，建议下一轮切换来源；只有用户未指定来源、不是辅助脚本结果，且预算允许时才可同轮执行。
5. 研究型人才、具身智能论文证据或学术图谱通道相关时，下一轮可切换到 `--mode research`。
