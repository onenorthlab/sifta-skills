...
§1–§3 的状态、证据等级、置信度是**模型内部推理**用的，决定一个人能不能进推荐人选、把握标多高。**渲染给用户时不要把这些术语写出来**，按 [templates/final-report.md](../templates/final-report.md) 翻译成白话：

- 证据等级 A → 「把握：高」；B → 「把握：中（建议先核实）」；C → 进「其它线索」。
