...
无论是否调用 CLI，最终输出仍要遵守同一证据和覆盖风险质量门。
...
| 参数 / 来源 | 默认地域处理 |
| --- | --- |
| 项目简报 | 在“假设”写 `默认地域/市场：中国/中文生态相关人才池优先（不做族裔推断）` |
...
如果人选地域未知或超出默认范围，输出时写入覆盖风险和 `nextVerification`。如果用户没有明确放宽
为全球人才池，缺公开中国/中文生态相关职业信号的人不能进入候选人或强线索分桶。
...
- 如果上一轮是仓库回退，下一轮不能把只有仓库 owner 的线索包装成强推荐；必须保留风险提示或转入来源地图。
- 默认地域未被用户放宽时，GitHub 结果必须核验公开中国/中文生态相关职业信号；没有该信号的人只能进入来源地图或待核验，不进候选人分桶。
...
研究来源地图用于 WAM/VLA、论文型人才、技术合伙人、科学顾问、推荐人入口等复杂画像。
...
- 再从来源地图转入 GitHub / LinkedIn 候选人渠道。
- paper、lab、company、repo、dataset 不能直接进入 `people`。
- 强 PI、founder、产业标杆不自动等于全职候选人；根据项目进入顾问推荐人池、产业标杆池或覆盖风险。

学术图谱属于研究来源地图的特殊路径，用于寻找基础模型、数学、代码、训练效率、
多模态等方向的早期研究人才和高潜研究工程师。CLI 中的 `--query` 是来源地图种子，
不是最终候选人查询。
...
| 来源 | 用法 | 边界 |
| --- | --- | --- |
| OpenAlex | works / authors / institutions / topics graph，扩展引用、共同作者、机构和研究方向 | 程序化来源地图来源；不能直接证明可招聘性 |
...
| 学术来源写进 `--sources`        | 混淆来源地图和候选人渠道    | 从 `--sources` 移除；改由宿主 Agent 做来源地图，CLI 只保留 `--mode research` 或候选人渠道 |
| 网页/来源地图线索进入 people   | 来源合同破坏                | 只进 `sourceMap` / `evidenceLog` / `warnings`                      |
| 仓库回退被强推                 | 质量门破坏                  | 保留覆盖风险；缺个人资料交叉证据时只作为来源地图入口              |
| 辅助脚本返回可用线索           | 执行预算破坏                | 本轮停止；不要继续网页、`gh`、浏览器、LinkedIn 或第二次 CLI        |
| 辅助脚本没有可用线索           | 召回不足                    | 报告覆盖风险；询问/建议下一轮，不静默换源                          |
