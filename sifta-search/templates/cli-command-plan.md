...
- 需要 CLI 的原因：<LinkedIn/X 连接器 / 稳定 JSON / 调用轨迹 / 人工反馈 / 回归验证 / 自动同步 Web history>
...
- CLI 只是薄连接器层：提供 Public API 工具访问、结构化 JSON、schema、trace、warnings、feedback 和默认 Web history 同步；不要把 skill workflow 再封装成 CLI 里的第二套 sourcing agent。
- `find-people` 默认同步 Web history；只有一次性排障、schema 检查、隐私 hard-stop、provider failure、0 人结果或用户明确要求不保存时，才追加 `--no-save`。
