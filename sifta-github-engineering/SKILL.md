...
description: >
    ...
    用户只要 1-3 个强线索时，先用内置小批量辅助脚本，不要手写多轮 `gh`、网页搜索或 CLI 查询；辅助脚本返回人数少于用户目标时也要停住并说明覆盖缺口，不为凑数继续查。
...
2. 区分计划和执行：用户要求“找人/给候选人/推荐/名单/几个/跑一轮/执行”时就是执行请求；只有用户明确要“怎么找/来源地图/先规划/升级门槛”时，才只输出 GitHub 找人入口、查询方案、证据门槛和下一步补证动作，不调用 CLI、网页搜索、浏览器或实时搜索 / 实时核验。
3. 小批量执行要及时停住：用户只要 1-3 个强线索时，不要手写 `gh`/网页循环；默认运行 `node scripts/small-batch-github.mjs --profile agent-runtime-cn --query "<英文技术能力画像>" --target-count 2 --json`。`target-count` 只表示最终展示上限，不是必须凑满的人数，也不是每次 GitHub API 拉取人数；helper 返回 0、1 或少于用户目标时，直接按 `../sifta-search/templates/final-report.md` 写成招聘报告：能推荐几人就说几人，弱线索放「其他线索」，0 人时不输出候选人表，不能为了补齐人数继续搜索。helper 默认从更大的 raw pool 里筛人（每条 user search 拉多名公开 profile，最多 hydrate 一批 profile），再做证据筛选和分类。辅助脚本通过 GitHub REST 读取公开数据，先做 people-first `search/users` 的 `location:` 变体召回，再做 broad user search 和 repo-first `search/repositories`，最后做 repo owner / contributor / merged PR 作者升级；默认不依赖固定 seed。固定 seed 只能在用户或 Agent 明确需要兜底时通过参数开启，并且只能作为找人入口补充，不能占满推荐人选。优先使用 `GH_TOKEN` / `GITHUB_TOKEN`，没有 token 时可匿名低额度请求。辅助脚本完成后，内部停止执行并整理成用户报告；最终答复不输出停止标记、命令、events、timing、target-count、脚本名、helper 字样或使用了哪个 skill，也不再查提交、issue、网页、GitHub API、`gh`、helper 返回链接或更多仓库。
    - 若辅助脚本失败、额度不足或宿主 Agent 改用手工 GitHub 搜索，仍必须保持同一门槛：默认中国/中文生态未被用户放宽时，推荐人选只能放有公开中国/中文生态职业信号的人。全球 repo contributor、全球项目 owner 或无地域/中文生态证据的人只能进“其他线索 / 全球备选 / 待补证入口”，不能写成“建议先核实”的候选人。
...
8. 使用 `--sources '["github"]'`；不要因为 0 个结果自动切到 LinkedIn 或 X。若需要外部补充，
   先把论文、项目、公司或个人线索写入其他线索和下一步补证动作。
9. 仓库、主题、包命中先是来源线索；有个人资料、贡献深度和身份交叉信号后才升级为候选人。
10. 输出按 `../sifta-search/templates/final-report.md`：推荐人选、相关作品 / 证据、其他线索、还要确认和下一步。

## 质量门

- 最终答复必须保留“还要确认”和下一步补证动作；辅助脚本停止时不要把停止标记、脚本名、命令、参数或运行过程写进用户答复。
- 小批量辅助脚本完成后必须停止；`target-count` 是展示上限，不是交付承诺。不能因为用户要 2 个而在 helper 返回 1 个后继续 `curl` GitHub API、`gh`、网页搜索、浏览器打开 profile、查 repo、查 commit 或查 org 来补第二人。正确做法是诚实输出 1 个可推进人选、若干待核验线索和下一轮补证建议。
- 仓库回退结果默认是弱线索或待确认，不等于强候选人。
- 重排按两轴、可解释：**主轴=证据强度**（持续贡献深度 / merged PR 深度 / 个人实现型 repo / 画像方向词命中），**次轴=地域 / 角色偏好**（中国/中文生态公开职业信号、当前角色精确命中）。次轴只在同一证据档内排序，**不得让 China-only 的弱证据候选反超证据强的人**，也不得靠抬高地域分把弱证据包装成强候选。证据档为 `weak` 的人即使满足地域门，也只进“其他线索”，不进推荐人选。每个候选要能打印命中了哪些信号（如 `contrib:50+ / personal-impl-repo:2 / core-term-match:2 / china-profile / evidence:strong`），供 Owner 复核。
...
- 只有同时有个人资料、工程项目、公司/经历或身份交叉信号时，才进入推荐人选。
- `priority=C`、职业工程证据弱、来源错误、仓库回退结果都只能进入其他线索或还要确认。
- 宿主原生 GitHub 搜索结果也要按同一质量门解释，不能因为不是 CLI 结果就降低证据要求。
- GitHub 优先不排斥学术或网页线索；这些线索只能帮助定位仓库或个人资料，不能替代候选人证据。
- 默认地域对找人入口是召回偏置，对推荐人选是升级门槛：优先看公开职业资料里的中国大陆、港澳台、中文教育/工作/社区、中国市场或中国相关机构/公司信号；不得凭姓名、照片、外貌、口音、族裔或国籍猜测。
- 用户没有明确放宽为全球人才池时，缺公开中国/中文生态相关职业信号的人只能作为来源地图线索、产业标杆或待核验对象，不能进入候选人或强推荐。
- 小批量最终推荐人选必须先过地域/市场门，再看工程证据。即使某人是核心 contributor，只要缺公开中国/中文生态职业信号，也不能作为默认中国人才池的“建议先核实”候选；只能写入“其他线索”，并提示需要用户确认是否放宽到全球人才池。
- 输出要用招聘视角，不要用技术尽调视角。推荐人选的核心列是“人选 / 招聘判断 / 为什么值得聊 / 相关作品或证据 / 还要确认 / 下一步”，repo、PR、commit 只是支撑证据，不是推荐理由本身。
...
3. repo owner / contributor / merged PR 作者升级：只有个人资料、贡献深度和工程证据足够时才进推荐人选。
...
不要让任何单个大仓库占满小批量预算；每个 repo 默认最多推荐 1 个人选，其他贡献者进入待核验人选或找人入口。不要让 `langgraph`、`openai-agents-python`、`autogen`、`deepeval` 等全球 seed 先占满小批量预算；它们只能作为用户明确放宽全球人才池后的全球标杆 / 来源兜底。降低只命中 `awesome-list`、curated list、资源集合、集成目录或泛 `AI tools` repo 的权重，并把原因写进还要确认。
